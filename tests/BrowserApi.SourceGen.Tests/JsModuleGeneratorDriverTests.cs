...
";
        ...
        // Param uses the unqualified Blazor type — `using Microsoft.JSInterop;` is already
        // in the generated file header, so the short form resolves. Path C skips emitting
        // any `JsModules.DotNetObjectReference`, so there is no ambiguity.
        Assert.Contains("DotNetObjectReference<TDotNetRef> dotNetRef", moduleSource);
...
namespace TestConsumer {
    ...
    [Fact]
    public void Generator_emits_JSDoc_summaries_on_records_and_properties() {
        // End-to-end verification that JSDoc written in .d.ts/.ts flows into the
        // generated C# as real <summary> XML docs — the IntelliSense pipeline the
        // preview.6 release is meant to unlock for consumers.
        var dts = @"
/** Configuration for a drag-and-drop context. */
export interface DragConfig {
...
";
        ...
        var recordSource = result.GeneratedTrees
            .First(t => t.FilePath.EndsWith("DragConfig.g.cs"))
            .GetText().ToString();

        // Interface-level summary from the author's JSDoc — not the boilerplate.
        Assert.Contains("<summary>Configuration for a drag-and-drop context.</summary>", recordSource);
        // Per-property summaries from author JSDoc.
        Assert.Contains("<summary>CSS selector for the container region.</summary>", recordSource);
        Assert.Contains("<summary>Pointer-move threshold in pixels before a drag starts.</summary>", recordSource);
        // The old "Maps to TypeScript property" boilerplate must NOT appear for properties
        // that had JSDoc. (It still appears as a fallback for properties without JSDoc —
        // covered by a separate test below.)
        Assert.DoesNotContain("Maps to TypeScript property <c>container</c>", recordSource);
    }

    [Fact]
    public void Generator_falls_back_to_boilerplate_summary_when_JSDoc_absent() {
        // Symmetric to the test above: properties without JSDoc should keep the
        // descriptive boilerplate so consumers still get *some* documentation, even
        // if it's generic. A blank summary would be worse than a useful fallback.
        var dts = @"
...
";
        var result = RunGenerator(dts, "wwwroot/js/plain.d.ts");

        var recordSource = result.GeneratedTrees
            .First(t => t.FilePath.EndsWith("Plain.g.cs"))
            .GetText().ToString();

        Assert.Contains("Maps to TypeScript property <c>a</c>", recordSource);
        Assert.Contains("Maps to TypeScript property <c>b</c>", recordSource);
    }

    [Fact]
    public void Generator_file_header_references_source_path() {
        // Every generated .g.cs should tell a reader where it came from. Opening an
        // unfamiliar .g.cs in the IDE should immediately show the .d.ts / .ts file
        // to edit — no hunting required.
        var dts = @"
export interface Cfg { x: number; }
export function foo(): void;
";
        var path = "wwwroot/js/src/my-module.d.ts";
        var result = RunGenerator(dts, path);

        foreach (var tree in result.GeneratedTrees
                     .Where(t => t.FilePath.EndsWith("Cfg.g.cs") || t.FilePath.EndsWith("MyModuleModule.g.cs"))) {
            var source = tree.GetText().ToString();
            Assert.Contains("Generated by BrowserApi.SourceGen from:", source);
            Assert.Contains(path, source);
            Assert.Contains("Do not edit this file directly", source);
        }
    }
